...
from langchain.retrievers import EnsembleRetriever          # fixed from langchain_classic
...
PDF_PATH       = "data/phfphandbook-compressed.pdf"
...
@st.cache_resource
def load_pipeline():
    st.info("⏳ Loading pipeline for the first time — this takes ~2 minutes...")
...
with st.sidebar:
    st.markdown("### About")
    st.markdown(
        "This chatbot answers questions based strictly on the **Philippine Family Planning Handbook (2023 Edition)**.\n\n"
        "It uses a hybrid BM25 + FAISS retrieval pipeline with a BGE cross-encoder reranker and Qwen3-4B as the language model."
    )
    st.markdown("---")
    st.markdown("**Example questions:**")
    st.markdown("- What are the different family planning methods?")
    st.markdown("- What are the side effects of oral contraceptive pills?")
    st.markdown("- Who are contraindicated for IUD use?")
    st.markdown("- What is the LAM method?")
    st.markdown("---")
        ...
        with st.spinner("Reading the handbook..."):
            ...
            if len(st.session_state.messages) > 1:
                ...
                try:
                    ...
                    search_query = prompt  # fallback to original if condense fails
